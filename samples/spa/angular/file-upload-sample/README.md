...
The entire lesson is in [`src/app/file.service.ts`](src/app/file.service.ts) —
the rest of the app is just UI around it.
...
- This is the **single-request** pattern for small files (capped at ~3.5 MB).
  ...
  flow.
- Running locally (`npm start`) uses an in-memory mock so you can exercise the
  whole UI offline.

## Scripts

- `npm start` – Start the local dev server (uses the in-memory mock store).
- `npm run build` – Type-check and build for production into `dist/app/browser/`.
...
1. Go to Power Pages home and click **Inactive sites**. Find **File Upload Sample (Angular)**
