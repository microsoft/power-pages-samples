// ---------------------------------------------------------------------------
// file.service.ts — THE LESSON OF THIS SAMPLE
//
...
// Everything lives in one injectable service so the AppComponent stays tiny —
// this service is the faithful translation of the React sample's fileService.ts.
@Injectable({ providedIn: 'root' })
export class FileService {
  // In-memory store backing the local dev mock so the UI is fully testable offline.
  private mockFiles: (UploadedFile & { documentbody: string })[] = [];
  private mockId = 1;
  ...
  async uploadFile(file: File): Promise<void> {
    ...
    const documentbody = await this.fileToBase64(file);
    const contactId = this.getContactId();

    if (isDevelopment) {
      await new Promise(r => setTimeout(r, 600));
      this.mockFiles.unshift({
        annotationid: `dev-${this.mockId++}`,
        filename: file.name,
        mimetype: file.type,
        filesize: file.size,
  ...
  async listFiles(): Promise<UploadedFile[]> {
    ...
    if (isDevelopment) {
      return this.mockFiles.map(({ documentbody: _ignored, ...rest }) => rest);
    }
  ...
  async downloadFile(annotationid: string): Promise<void> {
    let filename: string;
    let mimetype: string;
    let documentbody: string;

    if (isDevelopment) {
      const found = this.mockFiles.find(f => f.annotationid === annotationid);
  ...
  /** Delete a file (note). */
  async deleteFile(annotationid: string): Promise<void> {
    if (isDevelopment) {
      this.mockFiles = this.mockFiles.filter(f => f.annotationid !== annotationid);
      return;
    }
