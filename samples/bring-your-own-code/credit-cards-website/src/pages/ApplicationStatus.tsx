...
const ApplicationStatus: React.FC = () => {
  ...
  const portalEmail = extWindow.Microsoft?.Dynamic365?.Portal?.User?.email ?? "";
  const isAuthenticated = portalEmail !== "";
  ...
  useEffect(() => {
    if (isAuthenticated && portalEmail) {
      setEmail(portalEmail);
    }
  }, [isAuthenticated, portalEmail]);
