...
export default function CreateRequest() {
  ...
  async function submitRequest() {
    try {
      ...
      if (!created) return
      if (!created.id || !created.requestNumber) {
        setErrors({ submit: t('createRequest.apiNote') })
        return
      }

      const srId = created.id
      const srNumber = created.requestNumber
    ...
    <div className="page">
      <div className="container" style={ { maxWidth: 700 } }>
        ...
        <div className="card animate-in animate-in-3" style={ { padding: 32, marginTop: 24 } }>
          ...
          { (requestError || errors.submit) && step === 'contact' && (
            <div role="alert" className="form-error" style={ { marginTop: 8 } }>
              <p>{ t('createRequest.apiNote') }</p>
